...
| [Repository Streaming Structure](backlog/repository-streaming-structure.md) | High | | | Design repository as streaming system: tables (immutable/insert-only artifacts) with indexes (compact mutable lookups). Kafka-compatible changelog streams. Foundation-level impact on information lookup friction. |

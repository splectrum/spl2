...
// v6: Self-eval harness - configurable checks with report
...
function createExecution(runtime, options = {}) {
  ...
  async function invokeMethod(methodPath, args = {}) {
    ...
    // Safety self-eval: verify no modification of runtime/execution
    if (selfEval.safety) {
  ...
  // Print self-eval report
  function printReport() {
    return report.print();
  }

  return { invoke, getApiState, setApiArgs, qualityControl, printReport, executionId };
}
...
async function main() {
  console.log('========================================');
  console.log('  Console API v6 - Self-Eval Harness');
  console.log('========================================');
  console.log('');
  ...
  try {
    ...
    // Create execution context with self-eval
    console.log('[2] Creating execution with self-eval...');
    const exec = createExecution(runtime, {
      verbosity: 'debug',
      selfEval: {
        safety: true,  // check runtime/execution not modified
        qc: true       // check schemas
      }
    });
    console.log(`    Execution ID: ${exec.executionId.slice(0, 8)}...`);
    console.log(`    Self-eval: safety, qc`);
    console.log('');

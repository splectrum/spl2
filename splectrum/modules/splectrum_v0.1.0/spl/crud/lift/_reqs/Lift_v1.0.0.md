...
# Lift entire container from lower module layer
spl spl/consumer/lift --modules --dryRun
spl spl/consumer/lift --modules
```
...
| `--modules` | Lift container from lower module layer (bm_spl → work_module) |
| `--recursive` | With --modules: lift full hive (all descendant containers) |
...
Lifts container from a lower module layer (e.g., bm_spl) to work_module.

1. Search lower layers for container (app modules first, then splectrum modules)
2. Copy container files + facet directories (_lib, _schemas, _reqs, etc.)
3. Skip files that already exist in work_module (preserves local edits)
4. Result: container now in work_module, ready for editing/upgrade

With `--recursive`: lifts the full hive (all descendant containers).

Use case: Migrating legacy containers from bm_spl to work_module for modernization.
